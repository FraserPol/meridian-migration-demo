...
The demo is built: see `meridian-migration-demo/` in this folder. It's a working Next.js app (login, profile, watchlist), the Migration Copilot (AI SDK + AI Gateway, tool-calling a mocked legacy route inventory), Terraform for the AWS/HCP Vault infrastructure, and a GitHub Actions CI workflow. Its own `README.md` has setup steps, demo credentials, and known limitations — including what was hand-verified in a sandboxed build environment versus what still needs `terraform apply` with real cloud credentials.

**Still on you before this satisfies the take-home's submission requirements:** push `meridian-migration-demo/` to a public GitHub repo (commit history is already there, just needs a remote), then `vercel link && vercel deploy --prod` to get the public URL both the brief and this document currently have as placeholders.
