...
**Static shell (Partial Prerendering), added post-interview:** the app originally opted every authenticated route out of static generation wholesale (`export const instant = false`, no `Suspense` boundary), so every visitor — regardless of region — paid a full round trip to the Function/Secure-Compute region before seeing anything, even page chrome that never varies by identity. `app/dashboard/layout.tsx` and `app/dashboard/page.tsx` now split that: the nav frame and loading skeleton have no per-request `headers()`/session read of their own and prerender once, serving from Vercel's Edge in every region on `cacheComponents: true`; the session check, the signed-in user's email, and the DB-backed profile/watchlist reads are isolated in their own `Suspense`-wrapped server components, so only that slice streams from the Function region. `/dashboard/profile` and `/dashboard/watchlist` inherit the shared shell from the layout change alone (confirmed via `next build`'s route table, which now marks all three as Partial Prerender rather than fully Dynamic). The root `/` redirect page and `/admin/migration-copilot` intentionally keep the prior fully-dynamic approach — a pure redirect has no meaningful shell to prerender, and the admin surface wasn't in scope for this pass.
