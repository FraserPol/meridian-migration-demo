...
[`lib/ai/sandbox-validator.ts`](lib/ai/sandbox-validator.ts) takes the config the Copilot just generated and runs it in an isolated Firecracker microVM before the approver ever sees a verdict. The problem this solves: `generateMigrationConfig` is template-based and therefore predictable, but predictable isn't verified — and asking a human to approve routing config that nobody has ever executed is a weak thing to gate. A sandbox is specifically what makes this possible rather than a linter: `next.config.ts` exports `rewrites()` as a *function*, so the only way to know what route table it actually produces is to call it, and calling agent-produced code inside the app's own process is the thing you must never do (see the unescaped-interpolation note in [`lib/ai/migration-planner.ts`](lib/ai/migration-planner.ts), which becomes live code execution the day the inventory stops being a hardcoded array). The check distinguishes what it really did per file — `next.config.ts` **executed** with its rules asserted, `proxy.ts` only **parsed** because it imports `next/server`, `nginx.conf` a **structural** check — rather than collapsing them into one "validated" badge that would overstate the result. It fails open: no OIDC token, quota, or a cold-start blip yields `skipped` with a reason and the snippets still return. Observed cost in production is ~2s wall clock and ~2s billed CPU per validation; note that this is *not* currently counted against the daily spend cap, which only tracks model tokens.
